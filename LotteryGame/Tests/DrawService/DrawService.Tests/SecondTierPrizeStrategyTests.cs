namespace DrawService.Tests
{
    ...
    [TestFixture]
    public class SecondTierPrizeStrategyTests
    {
        private Mock<IConfiguration> configurationMock;
        private SecondTierPrizeStrategy strategy;

        [SetUp]
        public void Setup()
        {
            configurationMock = new Mock<IConfiguration>();
            configurationMock.Setup(x => x["Prize:SecondPrizeRevenueShare"]).Returns("0.3");
            configurationMock.Setup(x => x["Prize:SecondPrizeTicketShare"]).Returns("0.1");

            strategy = new SecondTierPrizeStrategy(configurationMock.Object);
        }
        ...
        {
            var draw = new Draw { Id = "draw1" };
            var remainingTickets = new List<string> { "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9", "t10" };
            ...
            var expectedWinnerCount = (int)Math.Round(remainingTickets.Count * 0.1m);
            expectedWinnerCount.Should().BeGreaterThanOrEqualTo(1);
            prizes.Should().HaveCount(expectedWinnerCount);

            var expectedAmount = (long)(totalRevenue * 0.3m / expectedWinnerCount);
